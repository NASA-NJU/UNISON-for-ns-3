...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("partition", "Partition method", partition);
  cmd.AddValue ("thread", "Number of threads", thread);
  cmd.Parse (argc, argv);

#ifdef NS3_MTP
  if (partition == "Bottleneck")
    {
      MtpInterface::Enable (thread, 62);
    }
  else if (partition == "Coarse")
    {
      MtpInterface::Enable (thread, 5);
    }
  else
    {
  ...
  if (partition == "Bottleneck")
    {
      T1 = CreateObject<Node> (1);
      T2 = CreateObject<Node> (1);
      R1 = CreateObject<Node> (2);
      S1.Create (10);
      S2.Create (20);
      S3.Create (10);
      R2.Create (20);
      ...
      for (uint32_t i = 0; i < S1.GetN (); i++)
        {
          S1.Get (i)->SetSystemId (systemId++);
        }
      for (uint32_t i = 0; i < S2.GetN (); i++)
        {
          S2.Get (i)->SetSystemId (systemId++);
        }
      for (uint32_t i = 0; i < S3.GetN (); i++)
        {
          S3.Get (i)->SetSystemId (systemId++);
        }
      for (uint32_t i = 0; i < R2.GetN (); i++)
        {
          R2.Get (i)->SetSystemId (systemId++);
        }
    }
  else if (partition == "Coarse")
    {
    ...
    {
      T1 = CreateObject<Node> ();
      T2 = CreateObject<Node> ();
