...
NodeContainer aggAll;

void SwitchToOptical (uint32_t interfaceId);
void SwitchToNormal (uint32_t interfaceId);

void
SwitchToOptical (uint32_t interfaceId)
{
  for (uint32_t i = 0; i < aggAll.GetN (); i++)
    {
      aggAll.Get (i)->GetObject<Ipv4> ()->SetDown (interfaceId);
    }
  Ipv4GlobalRoutingHelper::RecomputeRoutingTables ();
  Simulator::Schedule (Seconds (double (conf::interval) * 1 / 5), SwitchToNormal, interfaceId);
}

void
SwitchToNormal (uint32_t interfaceId)
{
  for (uint32_t i = 0; i < aggAll.GetN (); i++)
    {
      aggAll.Get (i)->GetObject<Ipv4> ()->SetUp (interfaceId);
    }
  Ipv4GlobalRoutingHelper::RecomputeRoutingTables ();
  Simulator::Schedule (Seconds (double (conf::interval) * 4 / 5), SwitchToOptical, interfaceId);
}
...
main (int argc, char *argv[])
{
  ...
  Ptr<Node> opticalSwitch = CreateObject<Node> (RANK (0));
  ...
  // connect the optical switch to aggregate switches
  p2p.SetDeviceAttribute ("DataRate", StringValue ("100Gbps"));
  p2p.SetChannelAttribute ("Delay", TimeValue (NanoSeconds (2000)));
  for (uint32_t i = 0; i < nPod; i++)
    {
      for (uint32_t j = 0; j < nAgg; j++)
        {
          string subnet = "20." + to_string (i) + "." + to_string (j + nEdge) + ".0";
          addr.SetBase (subnet.c_str (), "255.255.255.0");
          for (uint32_t k = 0; k < nEdge; k++)
            {
              NetDeviceContainer ndc = p2p.Install (opticalSwitch, agg[i].Get (j));
              red.Install (ndc);
              addr.Assign (ndc);
            }
        }
  ...
  Simulator::Schedule (Seconds (0), SwitchToNormal, conf::k);
